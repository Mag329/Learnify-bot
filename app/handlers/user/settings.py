...
async def enable_new_mark_notification_settings(callback: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        await callback.answer(
            f'Уведомления о новых оценках {"✅" if settings.enable_new_mark_notification else "❌"}\n\nПолучать уведомления о новых оценках',
            show_alert=True,
        )
...
async def enable_new_mark_notification_settings(callback: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        await callback.answer(
            f'Уведомления о новых ДЗ {"✅" if settings.enable_homework_notification else "❌"}\n\nПолучать уведомления о новых домашних заданиях',
            show_alert=True,
        )
        await callback.message.edit_text(
            text=callback.message.text,
...
async def skip_empty_days_schedule_settings(callback: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        await callback.answer(
            f'Пропускать пустые дни (расписание) {"✅" if settings.skip_empty_days_schedule else "❌"}\n\nНе показывать дни без уроков в расписании',
            show_alert=True,
        )
        await callback.message.edit_text(
            text=callback.message.text,
...
async def skip_empty_days_homeworks_settings(callback: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        await callback.answer(
            f'Пропускать пустые дни (ДЗ) {"✅" if settings.skip_empty_days_homeworks else "❌"}\n\nНе показывать дни без домашних заданий',
            show_alert=True,
        )
        await callback.message.edit_text(
            text=callback.message.text,
...
async def next_day_if_lessons_end_schedule_settings(callback: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        if settings:
            settings.next_day_if_lessons_end_schedule = (
                not settings.next_day_if_lessons_end_schedule
            )
            await session.commit()

        await callback.answer(
            f'Расписание на следующий день {"✅" if settings.next_day_if_lessons_end_schedule else "❌"}\n\nПоказывать расписание на следующий день, если уроки в этот день закончились',
            show_alert=True,
        )
        await callback.message.edit_text(
            text=callback.message.text,
...
async def next_day_if_lessons_end_homeworks_settings(callback: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        if settings:
            settings.next_day_if_lessons_end_homeworks = (
                not settings.next_day_if_lessons_end_homeworks
            )
            await session.commit()

        await callback.answer(
            f'ДЗ на завтра после уроков {"✅" if settings.next_day_if_lessons_end_homeworks else "❌"}\n\nПоказывать домашние задание на следующий день, если уроки в этот день закончились',
            show_alert=True,
        )
        await callback.message.edit_text(
            text=callback.message.text,
