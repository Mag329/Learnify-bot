...
async def subscription_settings_handler(callback: CallbackQuery):
    ...
    logger.info(f"User {user_id} opened subscription settings")
    
    text = "🎁 <b>Настройки подписки</b>"

    await callback.answer()
    await callback.message.edit_text(
        text, reply_markup=await kb.subscription_settings(callback.from_user.id)
    )
...
async def subscription_setting_auto_renew_handler(callback: CallbackQuery):
    user_id = callback.from_user.id
    logger.info(f"User {user_id} toggled auto-renew")
    
    async with await get_session() as session:
        result = await session.execute(
            db.select(PremiumSubscription).filter_by(user_id=callback.from_user.id)
        )
...
async def subscription_setting_auto_gdz_handler(callback: CallbackQuery):
    user_id = callback.from_user.id
    logger.info(f"User {user_id} opened auto-GDZ settings")
    
    async with await get_session() as session:
        result = await session.execute(
            db.select(Gdz).filter_by(user_id=callback.from_user.id)
        )
        gdzs = result.scalars().all()
        
        logger.debug(f"User {user_id} has {len(gdzs)} auto-GDZ configurations")
        ...
        await callback.message.edit_text(
            text=text,
            reply_markup=await kb.choice_subject(callback.from_user.id, "auto_gdz"),
        )
...
async def select_subject_auto_gdz_handler(callback: CallbackQuery, state: FSMContext):
    ...
    logger.info(f"User {user_id} configuring auto-GDZ for subject_id={subject_id}")

    text = f"⚡ <b>Настройки авто-ГДЗ</b>\n\n"

    async with await get_session() as session:
        result = await session.execute(
            db.select(Gdz).filter_by(
                user_id=callback.from_user.id, subject_id=subject_id
            )
        )
        ...
        if subject_gdz:
            ...
            logger.debug(f"Existing config found for user {user_id}, subject {subject_id}")

            await callback.answer()
            return await callback.message.edit_text(
                text=text,
                reply_markup=await kb.auto_gdz_settings(subject_gdz=subject_gdz),
            )
        else:
            ...
            logger.debug(f"No config found for user {user_id}, subject {subject_id}, creating new")
            ...
            await callback.answer()
            await callback.message.edit_text(
                text=text, reply_markup=kb.back_to_subscription_settings
            )
...
async def select_gdz_url_handler(message: Message, state: FSMContext, bot: Bot):
    ...
    logger.debug(f"User {user_id} entered URL: {url}")
    
    if "https://" not in url or "gdz.ru" not in url:
        logger.warning(f"User {user_id} entered invalid URL format: {url}")
        return await message.answer(
            "❌ <b>Неверный формат ссылки</b>\nСсылка должна начинаться с <i>https://gdz.ru</i>",
            reply_markup=kb.back_to_subscription_settings,
        )
    ...
    await bot.edit_message_text(
        ...
        text=text,
        reply_markup=kb.choose_search_by_auto_gdz,
    )
...
async def auto_gdz_change_search_by_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if search_by not in ["pages", "numbers", "paragraphs"]:
        logger.error(f"User {user_id} provided invalid search_by value: {search_by}")
        return await callback.answer(
            "❌ <b>Неверный формат данных</b>", show_alert=True
        )

    data = await state.get_data()
    logger.info(f"User {user_id} saving auto-GDZ config: subject={data['subject_id']}, search_by={search_by}")

    async with await get_session() as session:
        result = await session.execute(
            db.select(Gdz).filter_by(
                user_id=callback.from_user.id, subject_id=data["subject_id"]
            )
        )
        subject_gdz = result.scalar_one_or_none()
        ...
        if subject_gdz:
            ...
            logger.debug(f"Created new config for user {user_id}, subject {data['subject_id']}")
...
async def change_auto_gdz_handler(callback: CallbackQuery, state: FSMContext):
    ...
    logger.info(f"User {user_id} changing auto-GDZ config for subject_id={subject_id}")
